...
![Stein, Papier, Schere](../images/rockpaperscissors.png)

## Einleitung

In diesem Projekt möchten wir den Sensor „InspectorP61x“ nutzen, um gegen den Computer das Spiel „Stein, Papier, Schere“ zu spielen. Der Sensor mit SICK Nova soll so eingerichtet werden, dass er die verschiedenen Handzeichen mithilfe von KI erkennt und die Daten über eine Ethernet-Verbindung überträgt. 
Mithilfe von Python und Flask wird eine Webanwendung für das Spiel programmiert, bei der der Computer ein zufälliges Zeichen auswählt und das Handzeichen vom Sensor erkannt wird. Diese Spielelogik wird dann auf dem Raspberry Pi ausgeführt. 

## Voraussetzungen

Richten Sie den Sensor wie im Abschnitt [Erste Schritte](./vision_getting_started.md) beschrieben ein.

In dieser Anleitung wird erläutert, wie Sie das webbasierte Spiel in Betrieb nehmen können und was dafür installiert und vorbereitet werden muss. Insgesamt kann ein wenig Programmiererfahrung zwar nicht schaden, ist aber keine Voraussetzung für dieses Projekt. Die Bereitstellung von Daten über die Ethernet-Verbindung wird im Vision Starter Kit erläutert. 
...
|------------------------|------------------------|
| **SICK Vision Starter Kit**<br>InspectorP61x für die industrielle Bildverarbeitung <br> Die Daten werden über Ethernet bereitgestellt. | **Raspberry Pi 5**<br>Ist ein vollwertiger Computer, an den ein Monitor, eine Tastatur und eine Maus angeschlossen werden können.<br>Empfängt Daten vom Sensor und verarbeitet diese in einem Python-Programm.<br>Steuerung über [SSH](https://www.cloudflare.com/de-de/learning/access-management/what-is-ssh/)
...
Der Sensor lässt sich mithilfe der Halterung auf verschiedene Arten befestigen. So kann er beispielsweise auf einem Tisch befestigt und die Hand unter den Sensor gehalten werden. Alternativ wäre es auch möglich, den Sensor vor einer Wand zu platzieren und die Hand vor die Wand zu halten.
...
1. Zunächst müssen wir den Imager für das Betriebssystem installieren: [Raspberry Pi Imager herunterladen](https://www.raspberrypi.com/software/).). Öffnen Sie das Programm anschließend und wählen Sie Folgendes aus: das passende Raspberry-Pi-Modell, das richtige Betriebssystem (in diesem Fall 64-Bit) und die SD-Karte, auf die der Imager geschrieben werden soll. 
...
Bevor wir jedoch alles speichern können, müssen wir unter „Routen…“ das Kontrollkästchen „Diese Verbindung nur für Ressourcen im eigenen Netzwerk verwenden“ aktivieren. Siehe unten:
...
10. In der sich nun öffnenden Befehlszeile geben wir den Befehl „ifconfig“ ein. Dadurch erhalten wir alle Informationen zu den derzeit verwendeten Netzwerkadaptern und deren IP-Adressen usw.
...
6. Wenn Sie für den Sensor eine andere IP-Adresse als die in der Dokumentation angegebene festgelegt haben, müssen Sie diese in der Datei „client.py“ ändern. Das Gleiche gilt für den Port. 
...
Wir speichern die Einstellungen, kehren zum vorherigen Menü zurück und übernehmen die Einstellungen. Wenn eine Meldung erscheint, dass alle Daten auf der SD-Karte gelöscht werden, bestätigen wir dies. Der Installationsvorgang auf der SD-Karte sollte nun automatisch fortgesetzt werden. 
...
6. Klicken Sie anschließend im soeben geöffneten Menü auf „Kabelverbindung 1“.  Es sollte etwa so aussehen.
...
Wenn wir bis hierhin alles richtig gemacht haben, sollte die folgende Ausgabe angezeigt werden.
...
Da wir zur Umsetzung des Projekts nicht standardmäßige Python-Module benötigen, müssen wir diese zunächst installieren. Dazu können wir den in Raspbian integrierten Paketmanager verwenden. 

1. Zunächst sollten wir sicherstellen, dass unsere Systempakete auf dem neuesten Stand sind. Öffnen Sie dazu die Befehlszeile und geben Sie folgenden Befehl ein:
...
Nun sollte alles bereit sein, damit du das Spiel starten kannst.
...
- Handzeichenerkennung in SICK Nova konfigurieren
